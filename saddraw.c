...
int main() {
  ...
  struct matrix * transform=make_rotY(60);
  struct matrix * transform2=make_rotX(-90);
  struct matrix * move=make_translate(250,50,0);
  add_cylinder(edges, 0, -100, 0, 100, 100, 10);
  //print_matrix(edges);
  //matrix_mult(transform,edges);
  //matrix_mult(transform2,edges);
  matrix_mult(move,edges);
  //draw_polygons(edges, s, zb, view, light, ambient, areflect, dreflect, sreflect);
  draw_lines(edges, s, zb, c0);
  /*
  edges->lastcol=0;
  add_sphere(edges, 50, 250, 0, 50, 30);
  draw_polygons(edges, s, zb, view, light, ambient, areflect, dreflect, sreflect);*/
  display(s);
  free(edges);
  return 0;
...
struct matrix * generate_cylinder(double cx, double cy, double cz, double r, double h0, int step) {
  ...
  double x,height,z;
  int rot;
  for (height=cy;height<=cy+h;height++) {
    //printf("height: %lf\n",height);
